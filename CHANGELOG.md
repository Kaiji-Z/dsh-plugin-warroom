...
格式遵循 [Keep a Changelog](https://keepachangelog.com/en/1.1.0/)；版本号沿用项目自身的
里程碑命名（v1.0 → V18…，发布 npm 包前不以 semver 承诺）。
...
- **HQ 注册弹窗「不是项目里的弹窗」（元首实抓）** —— 点击 HQ 后弹出的注册工作区面板裸渲染 `war-modal`，缺背板包装：无 fixed 定位、无遮罩、不居中，以文档流内联在星域容器里。补齐全项目统一的 背板 + 点击停传播 + 焦点圈 三件套。
...
## [V18 – V18.7] - 2026-08-29 → 2026-08-30
...
## [V9.2 – V9.13] - 2026-08-25 → 2026-08-26
