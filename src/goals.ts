 ...
 *   参谋路径必须紧跟 disarm（红线：参谋 goal 永远 disarm）。
...
export interface GoalsFace {
  ...
  /** 幂等拆除武装（round driver 停驱——参谋状态机专用）。 */
...
/**
 * 参谋状态机开箱（L2 澄清期）：create 后**立即 disarm**——round driver
 * 永不驱动参谋（红线：参谋醒着的每轮都是插件唤醒的）。同样带同
...
export async function openDisarmedGoalForDirective(face: GoalsFace | undefined, agent: unknown, directiveId: string): Promise<string | undefined> {
  try {
    ...
    const view = await face.create(agent, { objective: `warroom 命令 ${directiveId} 澄清收敛（参谋状态机，disarm）`, maxGoalRounds: 1 })
    const ref = goalRefOf(view)
    if (ref === undefined) return undefined
    // create 默认 armed——参谋路径必须紧跟 disarm（CAS：用 create 返回的 view）。
