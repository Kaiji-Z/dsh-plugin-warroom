 ...
 * （战报/战利品/证据文件路径/验收结论/败因）折成续接代可读的链档案。
 *
 * 尺寸纪律（dossier 无上限的教训）：参谋版 cap 1500 字（最近 3 代详情 + 更老一代
 * 一行式）；指挥官征召版 cap 600 字；pivot 直插版 cap 400 字。
...
/** 最近 detailGens 代给详情（结论+战报摘要+产物路径），更老各代一行式。 */
...
/** 指挥官征召令压缩版（≤600 字）：世代一行式 + 末代详情 + 产物路径。 */
...
/** pivot 直插执行会话的父代速览（≤400 字：父代结论 + 产物 + 战报摘要）。 */
