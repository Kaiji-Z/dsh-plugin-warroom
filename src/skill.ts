...
任务落在哪个工作区？判定顺序：
1. **命令带战场标记行【战场：<路径>】的**（元首下达时显式选定）→ 无条件用该工作区，不许改道
2. **续接命令（deepen/retry/pivot）** → 默认发布到**父代任务的工作区**——战线跟着战场走，续接跨战场=另起一条新战线，只有命令明确要求换地点（如"在干净的复现仓里试"）才换，换了要在发布说明里点明
3. 元首在正文里点名的项目（"给 kaijibot 加……"→ 该项目目录）
4. 没点名 → **最近用过的工作区**（本会话刚发布过任务的地方）
5. 还定不了 → **当前打开的工作区**，或出决策卡让元首选（选项显示**项目名**，不显示路径——元首不需要知道什么叫路径）
