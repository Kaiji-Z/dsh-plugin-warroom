...
1. **目标规格=`.goal/SPEC-B1.md`（后端本职六件）与 `.goal/SPEC-B2.md`（提示词最小充分审查）**；历史 SPEC v1-v8 已退役（内容散见 DESIGN.md 各节与 git 历史，.goal/ 下文件已不存在）。动手前先过一遍本文件「迭代注意」坑录与 DESIGN.md 对应节。
2. **下列不变量是红线**：attemptId 令牌制、KillCredit 证据链、JSON-text 通道（dsh 丢 `type:'json'` 参数）、SSE revision-only、**板是读投影 + 唯一例外挂载**（浏览器端不提供改任务的写操作）、**用户输入只在指挥中心（左区）**。任何「优化」不得破坏它们。
...
| 文件 | 职责 |
|---|---|
| `index.ts` | 插件入口：服务装配、外勤小队征召器（conscriptor，含 B1-件⑤ orphan 落盘 orphans.json + patrol 死会话 rescue 段）、巡检 rescue（直接征召，无 LLM nudge）、引信生命周期 |
...
| `relay.ts` | **每命令一会话** relay（pending 无会话即建 `大副·<摘要>`）、命令引信（tickNow 秒级 + 15s 兜底）；征召令/转达模板正文在 prompts.ts（B1-件①，relay 不得再出现长文案） |
...
| `report-capture.ts` | 外勤组员回报事件解析纯函数（parseUnitReportEvent：嵌套 .data 优先/扁平退回，registerReportCapture 消费） |
| `skill.ts` | 大副起草法（warroom-bounty-drafting，编程注册——**对 apiProxy 会话不可见**，靠 relay 内嵌要点兜底，见 AGENTS 提示词资产纪律） |
| `persona.ts` / `units.ts` / `toml.ts` | 外勤小队 persona / 组员 roster / TOML 加载 |
| `commands.ts` | `/war` 斜杠命令（host 侧入口）：激活先于提示落队（顺序保证） |
| `config.ts` | 插件配置 schema：编制/重试/外勤小队上限、状态文件路径 |
| `schedule.ts` | 日常任务令 cron：5 段表达式解析 + 下次运行计算（错过即跳过，不回填） |
| `state.ts` | 全局战时状态 JSON（激活 + HQ 绑定 + 当前战役指针；历史只在事件日志） |
| `types.ts` | 领域类型（组员/战役事件/fold 状态），零 harness 依赖保持纯度 |
| `fold-cache.ts` | **B1-件③ JSONL 装载缓存**：mtime+size 指纹（append 必失效），events/directives/threads/planets 四路共用；读计数器 foldCacheProbe 机检 |

tests/ 48 个 .test.ts 与 src/ 一一对应；`scripts/verify.mjs` 是验收断言（含 bundle needle 检查），`scripts/seed-smoke.ts` 造演示板，`scripts/shoot-v7.py`/`shoot-v17.py`（Playwright 截图取证）、`scripts/exam-e2e.py`（实弹考题驱动）是浏览器侧工具（exam-v3/v4/v5/v15、shoot-board 等历史脚本已归档 `scripts/archive/`；起服前清空的只有隔离 `.smoke-state`——**绝不能无参跑 `seed-smoke.ts`**，默认状态目录是真实数据）。
...
- 完整坑录与现场案例见 DESIGN.md 各节与 git 历史。
...
插件经 junction 挂进 `~/.dsh/profiles/web/node_modules`（`pnpm add file:` 在本机装不进，junction + 手动依赖绕过）。改动 → `pnpm build` → 重启服务器生效。（本仓在 C:/ 与 D:/ 两盘互为镜像——仓内绝对路径按实际 checkout 替换。）

overlay 变体（`cordis.*.yml`）：`dev` 常规联调；`dev-on` 强制战时激活启动；`smoke` 隔离 statePath 演示数据（配 `scripts/seed-smoke.ts`）；`forensic` 取证隔离态（:3099）；`exp` 仅实验用。overlay 里插件必须**按包名**引用——file:// URL 条目会被 client bundle 扫描器漏掉客户端半边。
...
- **flags 默认全开政策（元首定，2026-08-25）**：开发期所有功能旗默认 on（`src/flags.ts` DEFAULT_ON_FLAGS + `runtimeFlags`），新功能**不再设旗**直接默认开。**例外（舰长令 2026-09-01）：`staff-auto-close` 默认 OFF——所有回报强制人工验收**，KillCredit 只做机械复核不自动收官；opt-in 走 `WARROOM_FEATURES=staff-auto-close` 或 overlay 的 `config.extraFeatures`（smoke overlay 即此法让 e2e 考题继续覆盖收官机制）；`WARROOM_FEATURES` 仅用于 `!name` 关闭个别旗或 opt-in `v5-spike`。正式版发布后恢复「每能力一 flag」流程。单测仍用 `readFeatureFlags`（纯显式，确定性）。
...
- 考题残留可清：`C:/Users/kaiji/vibecodingKJ/temp/e2e-exam-ws`（e2e 门工作区；旧 exam-wsA/B、exam-v3-ws 已清）；`scripts/seed-smoke.ts --clear` 可重置演示数据。
- **模拟舰桥（playground）协议（2026-08-25 起，常驻演示板）**：给舰长把玩 UI/操作的演示板。必须**停服 → 播种 → 起服**三步：运行中的服务器会用内存旧态落盘覆盖种子（已实测：起服后播种，几分钟后 directives.jsonl 被清空、板变空）。播种命令 `python scripts/seed-playground.py`（只动隔离 `.smoke-state`，与 shoot-v7.py Phase 0+C 同源：全要素演示板 + L1 计划待批命令），然后按本地起服节重启 cordis.smoke.yml 服。验证：`GET /warroom/api/board` 应返回 commands 非空且 3 分钟不消失。
- **CHANGELOG 纪律（2026-08-30 元首令，固化）**：每个版本交付必须在 `CHANGELOG.md` 顶层 `[Unreleased]` 下记一条——Keep a Changelog 1.1 格式（Added/Changed/Fixed/Removed 分组，一条=一项用户/开发者可感变化，验收证据随条目附注），风格参照 `LookatStudy/CHANGELOG.md`；**版本号一律 v0.x.y-N**（`v0.<里程碑>.<迭代>-<刀数>`，如 v0.18.9-6——元首令 2026-08-30 二改：pre-1.0 用 0 前缀，刀数进 `-N` 预发布段；package.json 去 v 前缀落 semver；同头版本内多刀以条目尾注 (0.18.9-N) 区分不另立头）；**发版门（元首令 2026-08-31）：平时交付只往 `[Unreleased]` 积条目、不动版本号（package.json 与版本头都不碰）——元首明说「发版」才落版本头+推进版本号+提交 package.json**。**发版动作=一条命令** `node scripts/release.mjs <version|milestone|iteration|knife> "message"`（verify 门→净树门→bump→commit→v* tag→push→CI OIDC 可信发布→轮询 npm 到活；workflow=`.github/workflows/publish.yml`，npm 侧 trusted-publisher binding=repo+publish.yml+**environment 留空**；**新包 bootstrap（勘误 2026-09-01：npm 没有 pending publisher 机制——那是 PyPI 的概念，此前记录有误）**：包必须先存在才能在 npmjs 包设置里配 Trusted Publisher，全新包名的首版须手动 `npm publish`（本地 npm login + pnpm build，**不打 tag、CI 不跑**——v0.19.0 同款路径；dsh-plugin-stardeck 0.20.1 即此法首发），落地后在包设置配好三元组，下一版起 tag→CI OIDC 接管；普通分支 push 永不发版；关键词双轨：milestone/major=里程碑 0.19.0、iteration/minor=迭代 0.18.10（刀清零）、knife/patch=刀 0.18.9-7；发版验证=重装 web profile——不走 `dsh plugin remove/add`（wrapper shell 出全局 pnpm，死于 profile 钉死的 pnpm lockfile）：改 `~/.dsh/profiles/web/package.json` 依赖版本→删 profile 的 node_modules+pnpm-lock.yaml→harness 根 `corepack pnpm -C <profile-dir> install`）。README 不维护版本史，只留指向 CHANGELOG 的链接。
- **提示词资产纪律（B1-件①，2026-08-31 固化）**：宿主侧提示词模板正文的单一资产源是 `src/prompts.ts`（relay 征召令/转达/战线档案段、外勤征召令组装 commanderOrderFor；persona.ts/skill.ts/chain-note.ts 仍是各自资产模块经其汇出）——**relay.ts/index.ts 不得再出现长文案字面量（verify 源级负针脚锁死）**。改任何提示词措辞=高风险变更：必须过 `tests/prompts-snapshot.test.ts` 全文快照门（先改源、跑 `WARROOM_UPDATE_SNAPSHOTS=1 node --import tsx --test tests/prompts-snapshot.test.ts` 再生成 fixtures、快照随改动一并提交评审）；GLM 网关可用时另跑 `pnpm verify:eval` 监督层。**出口协议段（war_claim 令牌/war_submit 证据/war_fail/war_comment/KillCredit）是不可裁剪内容**——快照测试点名断言，任何「瘦身」动了即 FAIL。
- git-bash curl POST 中文 JSON 会乱码入账——API 抽查一律走浏览器 fetch 或 node fetch。
- **执行者工具通道环境坑（stardeck 回流 2026-09-04）**：zcode 无头模式（`--prompt`）不加载项目 `.mcp.json`（MCP 桥只对 TUI 会话生效）；任何 agent 用 Windows 控制台 curl 拼 JSON 发工具调用会 GBK 乱码（U+FFFD 入账）——工具调用一律 node 脚本 + UTF-8 发请求。stardeck 侧在 daemon 层对 war_submit.report/war_publish 字段做乱码哨（U+FFFD 命中即回执 warning），宿主插件形态无此拦截点，靠此坑录预防。
...
| LLM 监督层 | **有（2026-08-24 整改；2026-08-31 首弹实弹 2/2 PASS）**——promptfoo（`eval/`，裁判 glm-5.2+隔离提示词，三维≥7+一票否决）；门命令 `pnpm verify:eval`，无网关环境变量时**显式 SKIP**；本机网关=LookatStudy `.env` 的 Z_AI_*（z.ai 兼容端点；**必须 `config.passthrough.thinking.type=disabled`**——网关默认开 thinking 吃光输出预算，坑录见 eval/README） | `eval/README.md`、`scripts/run-eval.mjs`，首跑取证 `.goal/evidence/b2-live/` |
...
- P1（部分清）：~~监督层首跑~~ **已清（2026-08-31）**——`pnpm verify:eval` 接 LookatStudy 网关首弹 2/2 PASS（正向 R3 轨迹 10/8/10 无否决、负向幽灵战报一票否决四条全中），实战阈值回填 eval/README.md。仍挂账：Playwright E2E 收进独立门、决策卡应答 API 化评估（涉宿主，只读边界）。
- P2：~~HTTP trace 端点~~ **已清（2026-08-31 B1-件②）**——`GET /warroom/api/trace?commandId=`（traceProjection 全事件时间线+引信/征召视角）。attention 类端点仍无（无此需求实例）。

### 项目参数（VERIFICATION.md §8，2026-08-24 填）
- 8.1 入口：宿主 CLI 起服（AGENTS.md 本地起服节）；触发 = `POST /warroom/api/commands`（`dashboard.ts:234`）与 `/war` 斜杠命令（`commands.ts:2-8`）；取 trace = `GET /warroom/api/board`（`:219`）+ `GET /warroom/api/trace?commandId=`（0.20.0 交付）+ 磁盘 JSONL 装载器。
- 8.2 测试基建：回归命令 `pnpm verify`（`package.json:17`）；回归集 tests/（48 个 .test.ts）；断言框架 node:test + node:assert/strict。
...
- 8.7 评测工具链：promptfoo 0.122（已装；2026-08-31 首弹实弹过门，接线与坑录 eval/README.md）。
