...
判据（全过 = PASS）：
  C1  代1 收官：task_closed + verdict 含 KillCredit 自动收官 + evidence.files
      含 v15-manifest（产物进了证据链——链档案的原料）
  C2  T1 因果纯净：token 是 8 位十六进制，且不出现于代1/代2 命令原文
      （token 由代1 LLM 现场生成——下游出现只能经链档案）
  C3  续接挂链：代2 directive continuation={mode:deepen, parentId:代1} + name=exam战线
  ...
  C6  链档案送达并使用（行为核心）：代2 任务书 brief 或 指挥官战报 提及
      'v15-manifest' 或 T1（参谋/指挥官不可能从命令原文或板摘要得知）
  C7  续在成果上（结果核心）：summary/v15-summary.md 存在且含 T1 数值
...
EV = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), '.goal', 'evidence', 'v15')
WS = 'C:/Users/kaiji/vibecodingKJ/temp/exam-v15-ws'
TAG1, TAG2 = 'V15代1考题', 'V15代2考题'
...
def campaign_events(task_id):
    path = os.path.join(EV, f'r15-campaign-{task_id}.jsonl')
    out = []
    with open(path, encoding='utf-8') as f:
        for line in f:
            line = line.strip()
...
board1 = load_json(os.path.join(EV, 'r15-board-gen1.json'))
board2 = load_json(os.path.join(EV, 'r15-board-gen2.json'))
created2 = [json.loads(l) for l in open(os.path.join(EV, 'r15-directives.jsonl'), encoding='utf-8') if '"directive_created"' in l]
...
T1 = open(os.path.join(EV, 'r15-token.txt')).read().strip()
...
check('C1c 代1 evidence.files 含 v15-manifest', any('v15-manifest' in f for f in files1), str(files1))
...
check('C3c 战线名 name=exam战线', d2.get('name') == 'exam战线', str(d2.get('name')))
...
needle = lambda s: ('v15-manifest' in s) or (T1 in s)
...
# C7 续在成果上
summary_path = os.path.join(EV, 'r15-summary.md')
...
for n in notes:
    print('note:', n)
print()
print('EXAM-V15: ' + ('PASS — 两代续接全链真实 LLM 走通，链档案注入有行为证据' if not fails else f'FAIL — {len(fails)} 项: {fails}'))
